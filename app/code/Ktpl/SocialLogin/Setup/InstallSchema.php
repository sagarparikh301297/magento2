...
class InstallSchema implements InstallSchemaInterface
{
    ...
    public function install(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (!$installer->tableExists('ktpl_social_customer')) {
            $table = $installer->getConnection()
                ->newTable($installer->getTable('ktpl_social_customer'))
                ->addColumn(
                    'social_customer_id',
                    Table::TYPE_INTEGER,
                    11,
                ...
                ->addForeignKey(
                    $installer->getFkName('ktpl_social_customer', 'customer_id', 'customer_entity', 'entity_id'),
                    'customer_id',
