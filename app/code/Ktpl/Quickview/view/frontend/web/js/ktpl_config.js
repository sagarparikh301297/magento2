define([
    'jquery',
    'mage/mage',
    'Ktpl_Quickview/js/jquery.magnific-popup.min'
], function ($) {
    "use strict";
    $.widget('ktpl.ktpl_config', {
        options: {
            productUrl: '',
            buttonText: '',
            isEnabled: false,
        ...
        renderButton: function () {
            ...
            if($widget.options.isEnabled == 1){
                $(productImageWrapper).each(function(){
                    ...
                    if (id_product) {
                        $(this).append('<div id="quickview-'+ id_product +'" class="ktpl-bt-quickview"><a class="ktpl-quickview" data-quickview-url="'+$widget.options.productUrl+'id/'+ id_product +'" href="javascript:void(0);" ><span>'+$widget.options.buttonText+'</span></a></div>');
                    }
                })
            }
        },
        ...
        _EventListener: function () {
            ...
            if($widget.options.isEnabled == 1){
                ...
                $('body, #layer-product-list').on('contentUpdated', function () {
                    $('.ktpl-bt-quickview').remove();
                    $widget.renderButton();
                });

                $(document).on('click','.ktpl-quickview', function() {
        ...
        openPopup: function (prodUrl) {
            var $widget = this,
                url = $widget.options.baseUrl + 'ktpl_quickview/index/updatecart';
            ...
            $.magnificPopup.open({
                ...
                callbacks: {
                    open: function() {
                      ...
                      $("iframe.mfp-iframe").contents().find("html").addClass("ktpl_loader");
                    },
